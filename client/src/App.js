...
function App() {
  ...
  const targetSighting = (sighting, id) => {
    const temp = birdSightings.map(s => s);
    const indexToUpdate = temp.map(s => s._id).indexOf(id);
    
    temp.splice(indexToUpdate, 1, sighting)
    setBirdSightings(temp)
  }
  ...
  return (
    <div>
      ...
      <SightingsGrid sightings={birdSightings} removeSighting={removeSighting} targetSighting = {targetSighting}/>
    </div>
  );
}
